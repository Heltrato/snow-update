...
public class ModMoveToTargetSink extends Behavior<Mob>{
    ...
    public ModMoveToTargetSink(){
        this(150, 250);
    }

    public ModMoveToTargetSink(int pMinDuration, int pMaxDuration){
    ...
    protected boolean checkExtraStartConditions(ServerLevel pLevel, Mob pOwner) {
        TitanYetiEntity pEntity = ((TitanYetiEntity)pOwner);
        if (pEntity.isOcupied())
            return false;
    ...
    protected boolean canStillUse(ServerLevel pLevel, Mob pEntity, long pGameTime) {
        TitanYetiEntity pOwner = ((TitanYetiEntity)pEntity);
        if ((pOwner.isCharging() || pOwner.isCrashing() || pOwner.isStartCharging()))
            return false;

        if (this.path != null && this.lastTargetPos != null) {
            Optional<WalkTarget> optional = pEntity.getBrain().getMemory(MemoryModuleType.WALK_TARGET);
            boolean flag = optional.map(ModMoveToTargetSink::isWalkTargetSpectator).orElse(false);
